...
params(1,1) = 0.16e-3+0.1e-3*randn(1);      % Ui
params(1,2) = 1.65e-3+0.1e-3*randn(1);      % Uf
params(1,3) = -6+1e-1*randn(1);             % Kexp
params(1,4) = 2.350e3+1e2*randn(1);         % Te
params(1,5) = 5.50e5+1e4*randn(1);          % fault_init_time
...
%p_steps = [0.2e-6, 2e-6, 1e-3, 5, 1000, 1e-3, 2.5e-4]; % higher acceptance rate (~40-60%), suggesting tuned properly
